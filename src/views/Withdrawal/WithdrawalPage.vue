<template>
  ...
  <div class="bg-[#1a45b1]">
    <div class="flex flex-col">
      <div class="flex p-[.2rem] bg-[</div>] border-b border-b-[#3A61C2]">
        <div class="flex w-full items-center">
          <div class="flex flex-col w-full">
            <div class="flex items-center gap-[.1rem]">
              <span class="text-white text-[.25rem]">Saldo da Conta</span>
              <span class="text-[#FFAA09] text-[.25rem]">{{
                withdrawConfigValue?.accountBalance
              }}</span>
              <img class="w-[.3rem]" src="/images/shuaxin.png" alt="" />
            </div>
            <div class="flex">
              <span class="text-[#6FA4EF] text-[.22rem]"
                >The amount of betting for the withdrawal is
                {{ userConfig?.money_unit
                }}{{ withdrawConfigValue?.betNum?.drawNeed }}</span
              >
            </div>
          </div>
          <div class="flex">
      ...
      <div class="flex p-[.2rem] bg-[#1a45b1] border-b border-b-[#3A61C2]">
        <div class="w-full">
          <div
            class="border border-[#FFF0BB] rounded-[.1rem] p-[.2rem] relative overflow-hidden"
          >
      ...
      <div class="flex flex-col w-full p-[.2rem] gap-[.5rem]">
        <router-link
          v-if="userBankValue?.content?.length === 0"
          @click="bankAccount.refetch()"
          to="/manageaccount"
        >
          <div
        ...
        >
          <div class="w-full relative">
            <div
              @click="bankOption"
              class="flex items-center justify-between w-full p-[.2rem]"
            >
              <div class="flex items-center gap-[.2rem]">
                <img
                  src="/images/bankCardInActive.png"
                  alt=""
                  class="w-[.3rem]"
                />
                <span class="text-[#A0C5FB] text-[.22rem]"
                  >{{ bankName }} ({{ cardNo }})</span
                >
              </div>
              <img
                src="/images/return.png"
                ...
                :class="
                  showBank
                    ? `w-[.25rem] rotate-90 transition-all`
                    : `w-[.25rem] -rotate-90 transition-all`
                "
              />
            </div>
            <div
              :class="
                showBank
                  ? `transition-all overflow-auto flex flex-col w-full py-[.2rem] h-auto max-h-[7rem] border-[.01rem] border-[#3A61C2] bg-[#05309F] left-0 rounded-[.1rem] absolute mt-[.2rem]`
                  : `hidden transition-all`
              "
            >
              <span
                v-for="(userBankData, index) in userBankValue?.content"
                @click="selectBankOption(userBankData)"
                :class="
                  userBankData?.id === bankId
                    ? `text-white font-bold p-[.2rem] bg-[#505050]`
                    : `text-white p-[.2rem]`
                "
                >{{ userBankData?.bankName }} ({{ userBankData?.cardNo }})</span
              >
            </div>
          </div>
        </div>
        ...
        >
          <div class="flex items-center gap-[.2rem] w-full">
            <span class="text-white text-[.23rem]">{{
              userConfig?.money_unit
            }}</span>
            <input
              v-model="money"
              class="h-[.5rem] text-white text-[.23rem] bg-transparent outline-none w-full"
              :placeholder="
                `Minimum amount ` +
                withdrawConfigValue?.minPickMoney +
                `, Maximum amount ` +
                withdrawConfigValue?.maxPickMoney
              "
              type="text"
            />
          </div>
        ...
        <div class="flex flex-col gap-[.1rem]">
          <div class="flex justify-between">
            ...
            <img
              v-if="viewPassValue"
              @click="viewPass"
              src="/profileImages/eye.png"
              class="w-[.4rem]"
            />
            <img
              v-if="!viewPassValue"
              @click="viewPass"
              src="/profileImages/eye-slash-1.png"
              class="w-[.4rem]"
            />
          </div>
          <VOtpInput
            ...
            :num-inputs="6"
            v-model:value="withdrawPassword"
            :should-auto-focus="false"
            :should-focus-order="true"
            @on-change="handleOnChange"
            :is-disabled="!userBankValue?.content?.length > 0"
        ...
        <button
          @click="withdraw()"
          class="w-full h-[.7rem] bg-[#FFF0BB] text-[#05309F] rounded-[.1rem] text-[.2rem]"
        >
          Confirmar
        </button>
      </div>
...
const isRate = useQuery({
  ...
  queryFn: async () => await axiosGet2("/api/native/v2/userBillIndex.do"),
  staleTime: 1000,
  select: (data) => {
...
const withdrawCash = useMutation({
  mutationFn: (payload) =>
    axiosPost3("api/native/v2/withdrawForWap.do", payload),
  onSuccess: (data) => {
