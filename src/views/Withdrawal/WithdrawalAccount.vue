<template>
  ...
  <div class="bg-[#1a45b1]">
      ...
      <div class="h-[calc(100vh-2.6rem)] w-full">
        ...
        >
          <div class="flex flex-item justify-between">
            <div class="flex items-center">
              <span class="text-white text-[.22rem]"
                >Conta de Retirada
                <span class="text-[#6FA4EF] text-[.22rem]">(0/4)</span></span
              >
            </div>
          ...
          <div class="flex flex-col overflow-auto gap-[.2rem] w-full">
            <div
              v-for="(bankAccountValue, indexes) in bankAccounts.filter(
                (entry) => entry.bankCode !== 'USDT'
              )"
              @click="setActiveAccount(indexes)"
              class="flex w-full items-center"
            >
              <div
                :class="
                  activeAccount === indexes
                    ? `flex border border-[#FFF0BB] rounded-[.15rem] p-[.2rem] pl-[.1rem] h-[.8rem] relative overflow-hidden w-full`
                    : `flex border border-[#1a68b1] rounded-[.15rem] p-[.2rem] pl-[.1rem] h-[.8rem] relative overflow-hidden w-full`
                "
              >
                <div class="flex items-center gap-[.2rem]">
                  <div class="flex items-center">
                    <img
                      src="/withdrawImages/bankIcon.png"
                      alt=""
                      class="w-[.6rem]"
                    />
                  </div>
                  <div class="flex flex-col leading-none gap-[.05rem]">
                    <span class="text-white text-[.22rem]"
                      >{{ bankAccountValue?.realName }}
                      <span class="text-[#6FA4EF] text-[.22rem]"
                        >/ {{ bankAccountValue?.bankName }}</span
                      ></span
                    >
                    <div class="flex items-center gap-[.1rem]">
                      <span class="text-[#6FA4EF] text-[.22rem]">{{
                        eyeOpen
                          ? bankAccountValue?.cardNo
                          : masAccountNum(bankAccountValue?.cardNo)
                      }}</span>
                      <img
                        v-if="eyeOpen"
                        @click="copyText(bankAccountValue?.cardNo)"
                        src="/copyIcons/Dior_copy.png"
                        alt=""
                        class="w-[.25rem]"
                      />
                    </div>
    ...
    <div
      class="flex items-baseline custom1 bottom-0 w-full p-[.2rem] bg-[#05309F] rounded-t-[.1rem]"
    >
      <div
  ...
  <!-- </WithdrawLayout> -->
  <AntModal :isOpen="bankModal" :componentPass="BankCardModal" :backGrounds="true" />
  <!-- <BankCardModal /> -->
