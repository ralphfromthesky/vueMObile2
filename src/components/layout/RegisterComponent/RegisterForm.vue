    ...
    <div class="flex flex-col w-full h-[7rem] overflow-auto">
      <div class="flex items-center justify-center gap-[.1rem]">
        <div class="flex items-center flex-col justify-center w-fit px-[.1rem]">
          <div class="flex items-center gap-[.1rem]">
            ...
            <span class="text-[.26rem] font-medium text-[#FFF0BB]"
              >Register</span
            >
          </div>
          <div
            class="w-full bg-[#FFF0BB] p-[.021rem] px-[.62rem] rounded-full mt-[.1rem]"
          ></div>
        </div>
      </div>
      <div class="flex flex-col mt-[.2rem] w-full">
        <div
          v-for="(regConfigData, index) in regConfig"
          :key="index"
          class="flex flex-col"
        >
          <div
            v-if="regConfigData?.eleName === 'rpwd'"
          ...
          >
            ...
            <input
              :type="
                regConfigData?.eleName === 'pwd' ||
                regConfigData?.eleName === 'rpwd'
                  ? `password`
                  : `text`
              "
              :placeholder="regConfigData.name"
              :name="regConfigData.eleName"
            ...
            <img
              v-if="regConfigData?.eleName === 'captcha'"
              :src="registerCode"
              alt=""
            ...
            <img
              v-if="
                ...
                regConfigData?.eleName === 'rpwd'
              "
              src="/profileImages/eye-1.png"
              alt=""
              class="w-[.4rem] absolute right-[.15rem]"
            />
      ...
      <div class="flex justify-between">
        <div class="flex items-center">
          ...
          />
          <label
            class="inline-block ps-[0.15rem] hover:cursor-pointer text-white text-[.22rem]"
        ...
        <span
          class="text-[.22rem] text-[#FFF0BB] cursor-pointer"
          @click="router.push('/passwordRetrieval')"
      ...
      <div class="flex justify-around">
        <span
          class="text-[#FFF0BB] text-[.22rem]"
          @click="() => router.push('/support')"
          >Suporte</span
        >
        <span class="text-[#FFF0BB] text-[.22rem]" @click="regGuest"
          >Jogar Grátis</span
        >
        <span
          class="text-[#FFF0BB] text-[.22rem]"
          @click="loginModal = !loginModal"
          >Login uma Conta</span
        >
      </div>
    </div>
  </form>
...
import { useGetUserInfo, changePassword } from "@/global/getUserInfo.js";
const {passwordStrength} = changePassword()
